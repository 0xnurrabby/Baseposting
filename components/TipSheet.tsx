...
const RECIPIENT = "0xB331328F506f2D35125e367A190e914B1b6830cF"; // checksummed EVM address
...
function encodeErc20Transfer(to: string, amount: bigint): `0x${string}` {
  ...
  return `0x${selector}${pad32(toNo0x.toLowerCase())}${pad32(amtHex)}`;
}

async function getDataSuffix(): Promise<string> {
  // Remote ESM import as required.
...
export function TipSheet({ open, onClose }: Props) {
  ...
  const canSend = React.useMemo(() => {
    try {
      ...
      return isAddress(rec) && BUILDER_CODE !== "TODO_REPLACE_BUILDER_CODE";
    } catch {
      return false;
    }
  }, []);
  ...
  async function sendTip() {
    try {
      ...
      setStage("preparing");
      // Pre-wallet animation (1–1.5s) REQUIRED.
      ...
      if (String(msg).toLowerCase().includes("rejected")) {
        toast("Tip cancelled");
      } else {
        toast.error(msg);
      }
