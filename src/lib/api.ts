...
export async function apiGenerateImage(identity: Identity, text: string) {
  return await postJson<{ ok: boolean; imageId: string; credits: number }>(
    '/api/generate-image',
    { ...identity, text }
  )
}

export function apiImageUrl(imageId: string) {
  return withOrigin(`/api/image?id=${encodeURIComponent(imageId)}`)
}
}
