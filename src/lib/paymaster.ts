...
export function buildWalletCapabilities(args: { dataSuffix?: any; enablePaymaster?: boolean }) {
  const caps: any = {}
  if (args?.dataSuffix) {
    const ds: any = args.dataSuffix
    // Some wallets (e.g., Farcaster Mini App host) validate `dataSuffix` as an object capability.
    // Accept either a pre-wrapped object or a raw hex string and normalize.
    caps.dataSuffix = typeof ds === 'string' ? { dataSuffix: ds } : ds
  }
