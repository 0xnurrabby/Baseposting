...
function normalizeWalletName(provider: any, fallback = 'Browser Wallet', detail?: Eip6963Detail) {
  ...
  if (typeof announcedName === 'string' && announcedName.trim()) return announcedName.trim()

  const providerInfoName = provider?.info?.name || provider?.providerInfo?.name || provider?.name
  if (typeof providerInfoName === 'string' && providerInfoName.trim()) {
    const clean = providerInfoName.trim()
    if (clean && clean.toLowerCase() !== 'ethereum provider') return clean
  }
...
function providerFingerprint(provider: any, name = '', detail?: Eip6963Detail) {
  const flags = [
  ...
  return [
    safeLower(detail?.info?.uuid),
    safeLower(detail?.info?.rdns),
    safeLower(detail?.info?.name),
...
function canonicalWalletBrand(name = '', rdns = '') {
  const hay = `${safeLower(name)} ${safeLower(rdns)}`
  if (hay.includes('metamask')) return 'metamask'
  if (hay.includes('rabby')) return 'rabby'
  if (hay.includes('okx')) return 'okx'
  if (hay.includes('coinbase') || hay.includes('base')) return 'coinbase'
  if (hay.includes('trust')) return 'trust'
  if (hay.includes('brave')) return 'brave'
  return hay.replace(/\s+/g, ' ').trim() || 'wallet'
}
...
function dedupeWallets(items: WalletOption[]) {
  ...
  for (const item of items.sort((a, b) => b.priority - a.priority)) {
    const brand = canonicalWalletBrand(item.name, item.rdns || '')
...
async function requestAccounts(provider: any) {
  ...
  const existing = (await request({ method: 'eth_accounts' }).catch(() => [])) as string[]
  if (Array.isArray(existing) && existing[0]) return existing
  return (await request({ method: 'eth_requestAccounts' })) as string[]
}
...
async function discoverEip6963Wallets(waitMs = 250): Promise<WalletOption[]> {
  ...
  const handler = (event: Event) => {
    ...
    const providerKey = providerFingerprint(provider, name, detail) || `${rdns}:${uuid}:${safeLower(name)}`
...
function getLegacyInjectedWallets(existing: WalletOption[] = []): WalletOption[] {
  ...
  const hasAnnounced = existing.length > 0
  const candidates = [
    ...(hasAnnounced ? [] : [anyWin?.ethereum, ...(Array.isArray(anyWin?.ethereum?.providers) ? anyWin.ethereum.providers : [])]),
    anyWin?.okxwallet,
    anyWin?.trustwallet,
  ...
  const seenBrands = new Set(existing.map((w) => canonicalWalletBrand(w.name, w.rdns || '')))

  candidates.forEach((provider: any, idx: number) => {
    const name = normalizeWalletName(provider)
    const brand = canonicalWalletBrand(name)
    const providerKey = providerFingerprint(provider, name) || `${safeLower(name)}:${idx}`
    if (seen.has(providerKey) || seenBrands.has(brand)) return
    seen.add(providerKey)
    seenBrands.add(brand)
    out.push({
