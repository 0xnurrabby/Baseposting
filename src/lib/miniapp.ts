...
export async function initMiniApp(): Promise<MiniAppState> {
  ...
  if (isInMiniApp) {
    try {
      ...
      // Different hosts/SDK versions can shape context differently.
      // Try the common locations for identity.
      const maybeUser = ctx?.user || ctx?.interactor || ctx?.viewer || null
      const fid = Number(maybeUser?.fid ?? ctx?.fid ?? ctx?.interactor?.fid ?? ctx?.viewer?.fid)

      if (maybeUser || Number.isFinite(fid)) {
        user = {
          ...(maybeUser || {}),
