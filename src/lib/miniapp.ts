...
export async function composeCast(args: { text: string; embeds?: string[]; channelKey?: string }) {
  // NOTE:
  // Some Mini App hosts have been observed to never resolve the composeCast
  // promise after the user posts and returns. That can leave the UI stuck in
  // a loading state. We guard with a timeout so the app stays responsive.
  ...
  // If the host never resolves, we treat it as "opened" and let the UI
  // continue; the real confirmation happens in the Farcaster composer.
