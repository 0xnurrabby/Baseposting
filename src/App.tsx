...
export default function App() {
  ...
  const busy = generatingText || generatingImage
  const canGenerate = useMemo(() => !busy && miniLoaded && isInMiniApp, [busy, miniLoaded, isInMiniApp])
  ...
  useEffect(() => {
    ...
    const img = localStorage.getItem('bp_with_image')
    setWithImage(img === '1')
  }, [])

  useEffect(() => {
    if (!mounted) return
    localStorage.setItem('bp_with_image', withImage ? '1' : '0')
  }, [withImage, mounted])
useEffect(() => {
  ...
  async function onGenerate(isRegen = false) {
    ...
    const requiredCredits = withImage ? 6 : 1
    ...
    try {
      ...
      // If image toggle is on, generate the image after we have the text.
      if (withImage) {
        setGeneratingImage(true)
        toast.message('Generating photo…')
        try {
          const imgOut = await apiGenerateImage(id, out.text)
          setImageUrl(imgOut.imageUrl)
          setCredits(imgOut.credits)
          toast.success('Photo ready 🖼️')
        } catch (eImg: any) {
          const status = (eImg as any)?.status
          if (status === 402) {
            setCredits((eImg as any)?.data?.credits ?? out.credits)
            toast.error('Not enough credits for photo')
          } else if (status === 429) {
            toast.error(eImg?.message || 'Image quota reached')
          } else {
            toast.error(eImg?.message || 'Photo generation failed')
          }
        } finally {
          setGeneratingImage(false)
        }
  ...
  async function onPostDirectly() {
    ...
    if (withImage && !imageUrl) {
      toast.message('Wait for the photo to finish generating')
      return
    }
    setPosting(true)
    try {
      ...
      const cast = await composeCast({ text: result, embeds: imageUrl ? [imageUrl] : undefined })
      if (!cast) {
            ...
            <CardHeader>
              <div className="flex items-start justify-between gap-3">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-full border border-zinc-200 bg-white/70 px-2 py-1 text-xs font-semibold text-zinc-700 backdrop-blur transition hover:bg-white dark:border-zinc-800 dark:bg-zinc-950/60 dark:text-zinc-200"
                >
                  <span>Photo</span>
                  <span className="relative inline-flex h-5 w-9 items-center rounded-full bg-zinc-200 p-0.5 dark:bg-zinc-800">
                    <span
                      className={
                  ...
                  <span className="text-[10px] opacity-80">{withImage ? '6c' : '1c'}</span>
                </button>
              </div>
            </CardHeader>
            <CardContent>
              <div className="flex flex-col gap-3">
                ...
                >
                  ...
                  {busy ? (generatingText ? 'Working…' : 'Generating photo…') : 'Generate'}
                </Button>
            ...
            <Card className="mt-6">
              ...
              <CardContent>
                ...
                {generatingImage && !imageUrl ? (
                  <div className="mb-4">
                    <div className="mb-2 text-xs font-semibold text-zinc-700 dark:text-zinc-200">Generating photo…</div>
                    <Skeleton className="h-56 w-full" />
                  </div>
                ) : null}

                {imageUrl ? (
                  <div className="mb-4 overflow-hidden rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950">
                    <img src={imageUrl} alt="Generated" className="h-auto w-full" loading="lazy" />
                  </div>
                ) : null}
                ...
                <div className="mt-4 flex flex-col gap-2 sm:flex-row">
                  <Button
                    variant="primary"
                    isLoading={posting}
                    disabled={!result || generatingText || generatingImage || (withImage && !imageUrl)}
