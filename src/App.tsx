...
import { buildWalletSendCallsCapabilities, getPaymasterUrl, isMethodNotSupportedError } from '@/lib/paymaster'
...
useEffect(() => {
  ...
  useEffect(() => {
    const auto = async () => {
      ...
      try {
        ...
        // Mini App wallets are typically already connected; avoid eth_requestAccounts which can be
        // unsupported or buggy in some Farcaster hosts.
  ...
  async function ensureWalletIdentity() {
    ...
    try {
      ...
      const provider: any = await getEthereumProvider()
      // In Farcaster Mini App hosts, calling eth_requestAccounts can be unsupported/buggy.
      // We prefer eth_accounts and only prompt outside of Mini App contexts.
      const existing = (await provider.request({ method: 'eth_accounts' })) as string[]
      let addr = existing?.[0]
      if (!addr && !isInMiniApp) {
      ...
      if (!addr) {
        throw new Error(
          isInMiniApp
            ? 'No wallet account available in this Mini App host. Try opening in Base app or a wallet-enabled browser.'
            : 'No wallet account'
        )
      }
  ...
  async function onGetCredit() {
    ...
    try {
      ...
      // Require Base Mainnet (best-effort).
      // Some Farcaster Mini App hosts have flaky provider method support; avoid hard-failing.
      if (!isInMiniApp) {
      ...
      const capabilities = buildWalletSendCallsCapabilities({ dataSuffix, paymasterUrl })
      if (capabilities) {
        callsPayload.capabilities = capabilities
      }

      // Base docs: wallet_sendCalls returns an object like { batchId, status },
      // but some wallets may return a plain string. Support both.
      let sendRes: any
      try {
        sendRes = await provider.request({
          method: 'wallet_sendCalls',
          params: [callsPayload],
        })
      } catch (e: any) {
        // Farcaster/Warpcast providers often don't support ERC-5792.
        if (isMethodNotSupportedError(e)) {
          // Fall back to a regular EOA transaction (user pays gas).

          toast.message('Gasless not supported in this wallet — sending a normal transaction (you will pay gas).')
          ...
          // Give the network a moment to index the tx
          ...
          if (verified?.pending) {
            toast.message(
              'Transaction is pending on Base. Please wait a few seconds and tap Get Credit again (or refresh).',
            )
            return
          }
  ...
  async function onSendTip() {
    ...
    try {
      ...
      const payload = {
        ...
        capabilities: buildWalletSendCallsCapabilities({ dataSuffix, paymasterUrl: getPaymasterUrl() }),
      }

      // ERC-5792 (gasless if supported)
      try {
