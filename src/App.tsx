  ...
  async function onGetCredit() {
    ...
    try {
      ...
      try {
        ...
        toast.message('Transaction sent... adding credit')
      } catch (e: any) {
        ...
        proofId = (await provider.request({ method: 'eth_sendTransaction', params: [tx] })) as string
        toast.message('Transaction sent... adding credit')
      }

      setCredits((prev) => Math.max(0, (prev ?? 0) + 1))
      const verified = await verifyCreditTxWithRetry(identity?.fid ? { ...identity, address: addr } : { address: addr }, proofId)
      setCredits(verified.credits)
      toast.success('Txn verified. +1 credit added')
    } catch (e: any) {
      if (isUserRejection(e)) {
...
async function waitForCallsTxHash(
  ...
  async function verifyCreditTxWithRetry(id: Identity, txHash: string) {
    return await apiVerifyTx(id, txHash)
  }
