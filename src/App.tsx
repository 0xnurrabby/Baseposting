...
import { buildWalletCapabilities } from '@/lib/paymaster'
...
function shouldFallbackToNormalGas(e: any) {
  const msg = String(e?.message || '').toLowerCase()

  // Many wallets throw a generic "not supported" or "capability" error when
  // paymaster sponsorship isn't available in that environment.
  if (msg.includes('capability') && msg.includes('paymaster')) return true
  if (msg.includes('does not support') && msg.includes('paymaster')) return true
  if (msg.includes('does not support') && msg.includes('gasless')) return true
  if (msg.includes('unsupported') && msg.includes('paymaster')) return true
  if (msg.includes('pm_sponsor')) return true

  // Paymaster/bundler can also return "denied" (billing not attached, allowlist, policy, etc.).
  // In those cases, we can still fall back to a normal gas tx if the user has ETH.
  if (msg.includes('request denied') || msg.includes('denied') || msg.includes('no billing')) return true

  return false
}

async function requireSomeBaseEth(provider: any, addr: string) {
  ...
  if (balance === 0n) {
    throw new Error('You need a tiny amount of ETH on Base to pay gas. Add a little Base ETH and retry.')
  }
}
...
useEffect(() => {
  ...
  async function onGetCredit() {
    ...
    try {
      ...
      // Prefer gas-sponsored tx via paymaster. If sponsorship isn't available (wallet doesn't support
      // it, billing/policy issues, etc.), fall back to a normal gas tx.
      const sponsoredCaps = buildWalletCapabilities({ dataSuffix, enablePaymaster: true })
      if (sponsoredCaps) callsPayload.capabilities = sponsoredCaps

      let sendRes: any
      try {
        sendRes = await provider.request({ method: 'wallet_sendCalls', params: [callsPayload] })
      } catch (e: any) {
        if (!shouldFallbackToNormalGas(e)) throw e

        // If sponsorship fails, require a tiny amount of Base ETH and retry without paymaster.
        await requireSomeBaseEth(provider, addr)
        const normalCaps = buildWalletCapabilities({ dataSuffix, enablePaymaster: false })
        const fallbackPayload: any = { ...callsPayload }
        if (normalCaps) fallbackPayload.capabilities = normalCaps
        else delete fallbackPayload.capabilities

        toast.message('Sponsor unavailable — using normal gas')
        sendRes = await provider.request({ method: 'wallet_sendCalls', params: [fallbackPayload] })
      }
      ...
      const txHash = await waitForCallsTxHash(provider, callsId)
...
async function waitForCallsTxHash(
  ...
  async function onSendTip() {
    ...
    try {
      ...
      // Prefer gas-sponsored tx via paymaster; fall back to normal gas if sponsorship isn't available.
      const sponsoredCaps = buildWalletCapabilities({ dataSuffix, enablePaymaster: true })
      if (sponsoredCaps) payload.capabilities = sponsoredCaps
      ...
      try {
        ...
        toast.message('Sponsor unavailable — using normal gas')
        await provider.request({ method: 'wallet_sendCalls', params: [fallbackPayload] })
      }
