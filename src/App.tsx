...
// NOTE: Tip recipient is NOT the credits contract.
const RECIPIENT = '0xe8Bda2Ed9d2FC622D900C8a76dc455A3e79B041f'
...
async function providerRequest<T = any>(provider: any, args: any, timeoutMs = 15000): Promise<T> {
  // Some Mini App wallet providers can hang; always time out.
  return await Promise.race([
    provider.request(args) as Promise<T>,
    new Promise<T>((_, reject) => setTimeout(() => reject(new Error('Wallet request timed out')), timeoutMs)),
  ])
}
...
export default function App() {
  ...
  useEffect(() => {
    const auto = async () => {
      ...
      try {
        ...
        // Try silent first.
        const existing = await providerRequest<string[]>(provider, { method: 'eth_accounts' }, 6000)
        if (existing?.[0]) {
        ...
        const accounts = await providerRequest<string[]>(provider, { method: 'eth_requestAccounts' }, 15000)
  ...
  async function onGetCredit() {
    ...
    try {
      ...
      const chainId = await providerRequest<string>(provider, { method: 'eth_chainId' }, 6000)
      ...
      const txHash = await providerRequest<string>(
        provider,
        {
  ...
  async function onSendTip() {
    ...
    try {
      ...
      if (chainId !== '0x2105') {
        try {
          await providerRequest(provider, { method: 'wallet_switchEthereumChain', params: [{ chainId: '0x2105' }] }, 15000)
        } catch (e: any) {
      ...
      // ERC-5792
      await providerRequest(provider, { method: 'wallet_sendCalls', params: [payload] }, 45000)
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                onClick={() => void ensureWalletIdentity()}
              >
                <Wallet className="h-4 w-4" />
                <span className="hidden sm:inline">
                  {identity.address ? shortAddress(identity.address) : 'Connect'}
                </span>
              </Button>
