...
useEffect(() => {
  ...
  async function onGeneratePhoto() {
    ...
    try {
      ...
      // Prefer an inline data URL for the on-page preview. This avoids a second fetch
      // that can fail on some hosts/devices even though the image generation itself succeeded.
      // Keep imageId so posting can still use a stable URL.
      const path = out.imageDataUrl || out.imageUrl || (out.imageId ? `/api/image?id=${encodeURIComponent(out.imageId)}` : '')
      setImageUrl(path.startsWith('data:') ? path : toAbsoluteUrl(path))
...
async function waitForCallsTxHash(
  ...
  async function verifyCreditTxWithRetry(id: Identity, txHash: string) {
    ...
    const err: any = new Error('Transaction sent. Credit will appear after block confirmation.')
    err.pendingTx = true
    throw err
  }
