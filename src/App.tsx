...
import { composeCast, getEthereumProvider, hapticImpact, hapticSelection, initMiniApp, listAvailableWallets, type MiniAppUser, type WalletOption } from '@/lib/miniapp'
...
useEffect(() => {
  ...
  async function connectWallet(option: WalletOption) {
    ...
    try {
      ...
      const provider: any = await getEthereumProvider(option.id, { isInMiniApp, client: miniClient })
      const existing = (await provider.request({ method: 'eth_accounts' }).catch(() => [])) as string[]
      const accounts = existing?.length ? existing : ((await provider.request({ method: 'eth_requestAccounts' })) as string[])
      const addr = accounts?.[0]
      if (!addr) throw new Error('No wallet account returned')
  ...
  async function onGetCredit() {
    ...
    try {
      ...
      // Detect whether the host wallet supports sponsored transactions via paymasterService (ERC-7677).
      // If supported, we can pass the CDP Paymaster URL in capabilities so the wallet can request paymaster data.
      // Ref: Base Account paymasterService capability.
      ...
      if (PAYMASTER_SERVICE_URL) {
        try {
          ...
          // ignore (method may be unsupported or blocked); we’ll just skip paymaster.
      ...
      const verified = await verifyCreditTxWithRetry(identity?.fid ? { ...identity, address: addr } : { address: addr }, txHash)
      setCredits(verified.credits)

      if (verified?.pending) {
        toast.message('Transaction is still indexing on Base. Refresh in a few seconds if the credit is not visible yet.')
        return
      }
...
async function waitForCallsTxHash(
  ...
  async function verifyCreditTxWithRetry(id: Identity, txHash: string) {
    let last: any = null
    for (let i = 0; i < 8; i++) {
      const res = await apiVerifyTx(id, txHash)
      last = res
  ...
  const isBrowserMode = !isInMiniApp
        ...
        <div className="mx-auto max-w-2xl px-4 py-10">
          {view === 'leaderboard' ? (
            ...
            <>
                ...
                <div>
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2 text-sm text-zinc-600 dark:text-zinc-400"><span>Stay consistent, Stay based, Post banger💙.</span><span className="rounded-full border border-zinc-200 bg-white/70 px-2 py-0.5 text-[11px] font-semibold text-zinc-700 backdrop-blur dark:border-zinc-800 dark:bg-zinc-950/60 dark:text-zinc-200">{isBrowserMode ? 'Browser mode' : 'Mini App mode'}</span></div>
                </div>
          ...
          <Card className="mt-6">
            <CardHeader>
              ...
              <div className="mt-1 text-xs text-zinc-600 dark:text-zinc-400">{isBrowserMode ? 'Browser mode uses detected injected wallets + Farcaster compose intent fallback.' : 'Mini App mode uses the in-app wallet when available.'}</div>
            </CardHeader>
            <CardContent>
