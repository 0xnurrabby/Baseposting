      ...
      if (miniLoaded && isInMiniApp && (identity.fid || identity.address)) {
        void (async () => {
          try {
            ...
            if (me.gifts?.total) {
              const firstMsg = me.gifts.applied?.[0]?.message
              const suffix = firstMsg ? ` — ${firstMsg}` : ''
              toast.success(`🎁 Admin gifted you ${me.gifts.total} credits${suffix}`)
            }
