...
function getValidDataSuffix(): string | undefined {
  const raw = (window as any).__ERC8021_DATA_SUFFIX__
  if (typeof raw !== 'string') return undefined
  const s = raw.trim()
  if (!s.startsWith('0x')) return undefined
  // must be even-length hex
  if (s.length % 2 !== 0) return undefined
  if (!/^0x[0-9a-fA-F]*$/.test(s)) return undefined
  // prevent absurdly large suffixes
  if (s.length > 4096) return undefined
  return s
}

function isCoinbaseProvider(provider: any): boolean {
  return Boolean(provider?.isCoinbaseWallet) || Boolean(provider?.isCoinbaseBrowser)
}
...
useEffect(() => {
  ...
  async function onGetCredit() {
    ...
    try {
      ...
      const dataSuffix = getValidDataSuffix()
      ...
      if (dataSuffix) {
        ...
        callsPayload.capabilities = { dataSuffix: { value: dataSuffix } }
      }
      }
