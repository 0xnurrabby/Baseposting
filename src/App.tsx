...
function isCoinbaseProvider(provider: any) {
  return !!(
    provider &&
    (provider.isCoinbaseWallet || provider.isCoinbaseBrowser || provider.isCoinbase)
  )
}
...
async function getWalletCapabilities(provider: any, address: string, chainId: `0x${string}` = '0x2105') {
  try {
    const res = await provider.request({
      method: 'wallet_getCapabilities',
      params: [address, [chainId]],
    })
    return res?.[chainId] || res?.['0x0'] || null
  } catch {
    return null
  }
}

function buildDataSuffixCapabilities(provider: any, caps: any, dataSuffix: string) {
  if (!isHexBytes(dataSuffix)) return null

  // Coinbase / Base Smart Wallet style capabilities report: { dataSuffix: { supported: boolean } }
  // When supported, Coinbase expects the capability to be shaped as { dataSuffix: { value: Hex } }.
  // If unsupported (or unknown for Coinbase), skip to avoid "Error generating transaction" UX.
  const isCoinbase =
    !!provider?.isCoinbaseWallet ||
    !!provider?.provider?.isCoinbaseWallet ||
    !!(window as any)?.ethereum?.isCoinbaseWallet

  const supported = caps?.dataSuffix?.supported
  if (typeof supported === 'boolean') {
    if (!supported) return null
    return { dataSuffix: { value: dataSuffix } }
  }

  if (isCoinbase) {
    // Unknown capability support in Coinbase environment → don't send it.
    return null
  }

  // Generic EIP-5792 Capability shape (best-effort/optional)
  return { dataSuffix: { value: dataSuffix, optional: true } }
}
...
useEffect(() => {
  ...
  async function onGetCredit() {
    ...
    try {
      ...
      // Use ERC-5792 (wallet_sendCalls) so the tx is a Smart Account transaction,
      // which is what Base Builder Code analytics currently supports.
      ...
      // NOTE: Coinbase/Base Smart Wallet preview sometimes fails when dataSuffix is included.
      // To avoid the "Error generating transaction" modal, we skip dataSuffix for Coinbase providers.
      if (!isCoinbaseProvider(provider)) {
        const caps = await getWalletCapabilities(provider, addr, '0x2105')
        const dsCaps = dataSuffix ? buildDataSuffixCapabilities(provider, caps, dataSuffix) : null
        if (dsCaps) {
          callsPayload.capabilities = dsCaps
        }
      }
      let txHash: string

      // Try EIP-5792 first (best UX, can enable sponsored/smart-account flows). If the provider
      // doesn't implement it in this host, fall back to a normal single transaction.
      // (EIP-5792 specifically calls out falling back to eth_sendTransaction for unsupported wallets.)
      try {
        // wallet_sendCalls returns an object like { batchId, status }, but some wallets return a plain string.
        let sendRes: any
        try {
          sendRes = await provider.request({ method: 'wallet_sendCalls', params: [callsPayload] })
        } catch (e: any) {
          // Some hosts are stricter about capability schemas. Retry once without capabilities.
          if (callsPayload?.capabilities) {
            delete callsPayload.capabilities
            sendRes = await provider.request({ method: 'wallet_sendCalls', params: [callsPayload] })
          } else {
            throw e
          }
        ...
        const callsId: string =
          typeof sendRes === 'string'
            ? sendRes
            : (sendRes?.batchId || sendRes?.id || sendRes?.result?.batchId || sendRes?.result?.id)

        if (!callsId || typeof callsId !== 'string') {
          throw new Error('Wallet did not return a valid batchId from wallet_sendCalls.')
        }

        toast.message('Tx sent. Waiting for confirmation…')
        txHash = await waitForCallsTxHash(provider, callsId)
        toast.message('Confirmed. Verifying…')
      } catch (e: any) {
        if (!isMethodNotSupported(e)) throw e

        // Fallback path requires Base ETH for gas. Some smart wallets can sponsor gas only via sendCalls.
        const balHex = (await provider.request({ method: 'eth_getBalance', params: [addr, 'latest'] })) as string
        if (BigInt(balHex) === 0n) {
          throw new Error(
            'Your wallet provider in this host does not support batch calls, and this account has 0 Base ETH for gas. Add a little ETH on Base and retry.',
          )
        }

        const tx = { from: addr, to: CONTRACT, value: '0x0', data }
...
// Try EIP-5792 first (best UX, can enable sponsored/smart-account flows). If not supported,
      // fall back to eth_sendTransaction.
      try {
        try {
          await provider.request({ method: 'wallet_sendCalls', params: [callsPayload] })
        } catch (e: any) {
          // Retry once without capabilities (attribution is best-effort).
          if (callsPayload?.capabilities) {
            delete callsPayload.capabilities
            await provider.request({ method: 'wallet_sendCalls', params: [callsPayload] })
          } else {
        ...
        const tx = { from: addr, to: USDC_CONTRACT, value: '0x0', data }
