...
useEffect(() => {
  ...
  async function connectWallet(option: WalletOption) {
    ...
    try {
      ...
      // Stop the button spinner immediately after a successful wallet connect.
      // Account/credit hydration can finish in the background.
      setWalletConnecting(false)
  ...
  async function onGetCredit() {
    ...
    try {
      ...
      const optimisticCredits = Math.max(0, (credits ?? 0) + 1)
      setCredits(optimisticCredits)
      toast.success('Txn sent. +1 credit added')

      // Release the UI immediately. Server-side dedupe still keeps awards safe.
      setGettingCredit(false)
      void verifyCreditTxWithRetry(identity?.fid ? { ...identity, address: addr } : { address: addr }, proofId)
        .then((verified) => {
          setCredits(verified.credits)
        })
        .catch(() => {
          // Keep the optimistic credit and quietly resync later.
          setTimeout(() => {
            void refreshMe()
          }, 1200)
        })
