...
import { Copy, Moon, Send, Sparkles, Sun, Wallet, HandCoins, X, Image as ImageIcon, Palette, UserPlus } from 'lucide-react'
...
import { composeCast, getEthereumProvider, hapticImpact, hapticSelection, initMiniApp, type MiniAppUser } from '@/lib/miniapp'
...
async function onCreateWallet() {
  ...
  try {
    ...
    const mod = await import('@/lib/baseAccount')
    const { provider, address } = await mod.connectBaseAccount({
      appName: 'BasePosting',
      appLogoUrl: `${SITE_URL.replace(/\/$/, '')}/assets/icon-1024.png`,
      chainIds: [8453],
    })
    activeProviderRef.current = provider
    setIdentity((prev) => ({ ...prev, address }))
    toast.success('Base Account connected')
  } catch (e: any) {
    if (isUserRejection(e)) {
      toast.message('Wallet request cancelled')
      return
    }
    toast.error(e?.message || 'Failed to create/connect wallet')
  } finally {
    setWalletConnecting(false)
  }
}
