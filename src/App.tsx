...
useEffect(() => {
  ...
  async function onGetCredit() {
    ...
    try {
      ...
      if (balance === 0n) {
        throw new Error(
          'You need a tiny amount of ETH on Base to pay gas (even for 0 value). Add a little Base ETH and retry.',
        )
      }
      ...
      if (dataSuffix) {
        callsPayload.capabilities = { dataSuffix: { value: dataSuffix, optional: true } }
      }


      let txHash: string

      // Prefer EIP-5792 batch calls when the wallet supports it, otherwise fall back
      // to a normal single transaction (eth_sendTransaction).
      const canUse5792 = await supportsEip5792Atomic(provider, addr, '0x2105')
      ...
      if (canUse5792) {
        ...
        try {
          ...
          // Some wallets (e.g., different Mini App hosts) can be stricter about capability schemas.
        ...
        // Wallet doesn't support EIP-5792 methods in this host (common in some Mini App clients).
        // Send a single transaction instead.
        const tx = {
          from: addr,
          to: CONTRACT,
          value: '0x0',
...
async function supportsEip5792Atomic(provider: any, address: string, chainId: string = '0x2105') {
  // EIP-5792 wallets should expose wallet_getCapabilities; if it's missing we treat as unsupported.
  try {
    const res = await provider.request({
      method: 'wallet_getCapabilities',
      params: [address, [chainId]],
    })
    const status = res?.[chainId]?.atomic?.status
  ...
  async function onSendTip() {
    ...
    try {
      ...
      if (canUse5792) {
        ...
        // Fallback to a single transaction when wallet_sendCalls is not supported.
        const tx = {
          from: addr,
          to: USDC_CONTRACT,
          value: '0x0',
