...
import { Copy, Image as ImageIcon, Moon, Send, Sparkles, Sun, Wallet, HandCoins, X } from 'lucide-react'
...
import { apiGenerate, apiGenerateImage, apiImageUrl, apiMe, apiShareAward, apiVerifyTx, type Identity } from '@/lib/api'
...
// Some providers are flaky when we pass dataSuffix as a wallet_sendCalls capability,
// so we append it ourselves and avoid capabilities entirely.
...
async function onGenerateImage() {
  ...
  if (credits === null) {
    ...
    try {
      ...
      setShareEligible(me.share.ok)
      setTodayUtc(me.share.todayUtc)
      currentCredits = me.credits
    } catch (e: any) {
      toast.error(e?.message || 'Failed to load credits')
      return
    } finally {
  ...
  if (currentCredits < 5) {
    toast.error('Need 5 credits for photo')
    return
  }
  ...
  try {
    ...
    setImageUrl(apiImageUrl(out.imageId))
    toast.success('Photo cooked ✅')
  } catch (e: any) {
    const status = (e as any)?.status
    if (status === 402) {
      setCredits((e as any)?.data?.credits ?? 0)
  ...
  async function onPostDirectly() {
    ...
    try {
      ...
      void composeCast({ text: result, embeds: imageUrl ? [imageUrl] : undefined })
    } catch (e: any) {
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <Button
                    ...
                    className={[
                      ...
                      'shadow-lg ring-2',
                      result
                        ? 'shadow-emerald-500/25 ring-emerald-500/25'
                        : 'shadow-red-500/25 ring-red-500/25',
                    ].join(' ')}
                  >
                    <ImageIcon className="h-4 w-4" />
                    {result ? 'Generate photo' : 'Generate photo'}
                  </Button>
                </div>
              </CardHeader>
              <CardContent>
                {imageUrl ? (
                  <div className="mb-4 overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
                    <img src={imageUrl} alt="Generated" className="h-auto w-full" />
                  </div>
                ) : null}
