...
export function LeaderboardPage(props: {
  identity: Identity
  meProfile?: { displayName?: string; username?: string; pfpUrl?: string } | null
  dark: boolean
  onToggleTheme: () => void
  onClose: () => void
}) {
...
const mergedRow = (row: LeaderboardRow): LeaderboardRow => {
  const fid = props.identity?.fid
  const me = props.meProfile
  if (me && fid && row?.fid === fid) {
    return {
      ...row,
      displayName: me.displayName || row.displayName,
      username: me.username || row.username,
      pfpUrl: me.pfpUrl || row.pfpUrl,
    }
  }
  return row
}

const displayEntries = useMemo(() => entries.map(mergedRow), [entries, props.identity?.fid, props.meProfile])
