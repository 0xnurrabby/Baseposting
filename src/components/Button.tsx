...
type ButtonProps = React.ButtonHTMLAttributes<HTMLButtonElement> & {
  variant?: 'primary' | 'secondary' | 'ghost' | 'danger' | 'attention' | 'success'
  isLoading?: boolean
}
...
export function Button({ className, variant = 'primary', isLoading, disabled, children, ...props }: ButtonProps) {
  const styles = {
    ...
    // High-attention CTA (red). Designed to look good in both themes.
    attention:
      'bg-gradient-to-r from-rose-600 via-red-600 to-orange-500 text-white shadow-md hover:brightness-110 active:brightness-95 dark:shadow-black/20',
    // Success CTA (green). Designed to look good in both themes.
    success:
      'bg-gradient-to-r from-emerald-600 via-green-600 to-lime-500 text-white shadow-md hover:brightness-110 active:brightness-95 dark:shadow-black/20',
  } as const
