...
function pfpProxy(url: string | null | undefined) {
  const u = String(url || '').trim()
  if (!u) return ''
  return `/api/pfp?u=${encodeURIComponent(u)}`
}
                  ...
                  <div key={e.member} className="flex items-center justify-between gap-3 py-3">
                    <div className="flex min-w-0 items-center gap-3">
                      ...
                      <div className="relative h-9 w-9 shrink-0 overflow-hidden rounded-full bg-zinc-200 dark:bg-zinc-800">
                        {e.pfpUrl ? (
                          <img src={pfpProxy(e.pfpUrl)} alt="" className="h-9 w-9 rounded-full object-cover" referrerPolicy="no-referrer" />
                        ) : null}
                      </div>
