...
// Goal: avoid boring paragraph blocks. Prefer visible structure (bullets, quotes, symbols).
const FORMAT_TEMPLATES: FormatTemplate[] = [
  {
    key: 'one_liner',
    name: 'One-liner',
    desc: 'One punchy line. Can use 1 emoji or ":)". No line breaks.',
    hardRules: ['Exactly 1 line.', 'No blank lines.', 'No multi-sentence paragraph.'],
    targetChars: '≤ 170 chars',
  },
  {
    key: 'split_punch',
    name: 'Split punch',
    desc: '3 short lines that read like a punchy cadence. NOT a paragraph.',
    hardRules: ['Exactly 3 lines.', 'No blank lines.', 'Each line should be short (≈ 4–10 words).', 'Include at least one visible marker like ":)", "→", or "•".'],
    targetChars: '≈ 180–260 chars',
  },
  {
    key: 'hook_bullets',
    name: 'Hook + bullets',
    desc: '1 hook line, then 3 bullets using "-" or "•".',
    hardRules: ['Line 1 is a hook.', 'Optionally add one blank line.', 'Then 3 bullets.', 'Use "-" or "•" bullets.'],
    targetChars: '≈ 240–420 chars',
  },
  {
    key: 'mini_checklist',
    name: 'Mini checklist',
    desc: 'A tiny checklist (3 items). Looks structured, not paragraph-y.',
    hardRules: ['Title line.', 'Then exactly 3 lines starting with "- [ ]".'],
    targetChars: '≈ 220–360 chars',
  },
  {
    key: 'blockquote_react',
    name: 'Quote + reaction',
    desc: 'A short quote line starting with ">", then 2–3 lines reacting to it.',
    hardRules: ['Line 1 starts with ">"', 'Total 3–4 lines.', 'No blank lines.'],
    targetChars: '≈ 240–420 chars',
  },
  {
    key: 'q_and_a',
    name: 'Q&A',
    desc: 'Question + answer, with a tiny kicker line.',
    hardRules: ['Line 1 must end with "?"', 'Line 2 answers it.', 'Line 3 is a short kicker (≤ 7 words).', 'Include one marker like ":)" or "→" somewhere.'],
    targetChars: '≈ 200–320 chars',
  },
  {
    key: 'mini_thread',
    name: 'Mini thread',
    desc: 'Numbered micro-thread: 1) 2) 3) lines. No paragraph.',
    hardRules: ['Exactly 3 lines.', 'Each line starts with "1)" or "2)" or "3)".'],
    targetChars: '≈ 260–460 chars',
  },
  {
    key: 'dialogue',
    name: 'Dialogue',
    desc: '2–4 lines like a quick convo (A:/B:).',
    hardRules: ['2 to 4 lines.', 'Each line starts with "A:" or "B:".'],
...
const STYLE_SEEDS = [
  ...
  'structured notes (bullets/checklist)',
]
...
function validateFormat(text: string, fmt: FormatTemplate): boolean {
  ...
  // Ensure structured formats actually contain visible structure.
  const hasBullet = /(^|\n)\s*[-•]\s+/.test(out)
  const hasChecklist = /(^|\n)\s*-\s*\[ \]\s+/.test(out)
  const hasQuote = out.trimStart().startsWith('>')
  const hasNumbered = /(^|\n)\s*[1-3]\)\s+/.test(out)
  const hasDialogue = /(^|\n)\s*[AB]:\s+/.test(out)
  const hasMarker = hasBullet || hasChecklist || hasQuote || hasNumbered || hasDialogue || /(:\)|\u2192|•)/.test(out)

  if (fmt.key === 'hook_bullets') return nonEmpty.length >= 4 && (hasBullet || /(^|\n)\s*•\s+/.test(out))
  if (fmt.key === 'mini_checklist') return nonEmpty.length === 4 && hasChecklist
  if (fmt.key === 'blockquote_react') return nonEmpty.length >= 3 && nonEmpty.length <= 4 && hasQuote
  if (fmt.key === 'mini_thread') return nonEmpty.length === 3 && hasNumbered
  if (fmt.key === 'dialogue') return nonEmpty.length >= 2 && nonEmpty.length <= 4 && hasDialogue
  if (fmt.key === 'split_punch') return nonEmpty.length === 3 && hasMarker
  if (fmt.key === 'q_and_a') return nonEmpty.length === 3 && /\?$/.test(nonEmpty[0].trim()) && hasMarker
...
async function openaiGenerate(args: {
  ...
  const system = [
    ...
    'Make the post visually readable: use line breaks and structure (bullets, > quote, 1) 2) 3), checklist, A:/B:) when the format asks for it.',
  ...
  try {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      try {
        ...
        // Enforce visible structure so we don’t get boring paragraph blocks.
