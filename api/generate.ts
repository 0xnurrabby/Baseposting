...
function fastLocalGenerate(args: {
  ...
  const recent = new Set(args.recentTexts.map((t) => t.toLowerCase().trim()))
  const fallback = pool.find((t) => {
    const x = t.toLowerCase().trim()
    return x && !recent.has(x) && x.length >= 40 && x.length <= 220 && !x.includes('http')
  }) || 'Base keeps the cost low enough that shipping feels fun again.'

  if (args.style.id === 'contrast') {
    return clampChars(postProcessOutput(`Then -> shipping onchain felt slow and expensive.
Now -> Base makes it light enough to just build and post.`), args.style.maxChars)
  }
  if (args.style.id === 'question-hook') {
    return clampChars(postProcessOutput(`What feels easier to ship on Base now than it did six months ago?

Low fees help... but the real win is how fast you can loop.`), args.style.maxChars)
  }
  if (args.style.id === 'checklist') {
    return clampChars(postProcessOutput(`- open Base
- ship the small thing
- post before you overthink it
- repeat tomorrow`), args.style.maxChars)
  }
  return clampChars(postProcessOutput(fallback.includes('Base') ? fallback : `${fallback}

Base makes the loop feel lighter.`), args.style.maxChars)
}
...
async function openaiGenerate(args: {
  ...
  let resp: Response
  try {
    resp = await openaiChatWithTimeout(body, apiKey, 5000)
  } catch {
...
async function openaiRewriteAddBase(args: {
  ...
  let resp: Response
  try {
    resp = await openaiChatWithTimeout(body, apiKey, 2500)
  } catch {
