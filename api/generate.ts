...
const STYLE_DECK: StyleDeck[] = [
  ...
  {
    id: 'tldr-quote',
    label: 'TL;DR with blockquote',
    formatGuide: '2–4 bullets OR 1 short blockquote (>) + 2 lines of commentary. Keep it crisp.',
    anglePrompts: ['security lesson', 'market weirdness', 'product lesson'],
    maxTokens: 170,
    maxChars: 420,
  },
  {
    id: 'checklist',
    label: 'Checklist',
    formatGuide: 'Use 3–5 checklist lines like "- [ ]" or "-". No long paragraphs.',
  ...
  },
  {
    id: 'mini-thread',
    label: 'Mini thread',
    formatGuide: 'Numbered 1) 2) 3). Each line short. Final line = takeaway. No “thread” word.',
    anglePrompts: ['simple mental model', 'why it matters', 'what to try next'],
    maxTokens: 190,
    maxChars: 520,
  },
  {
    id: 'dialogue',
    label: 'Dialogue',
    formatGuide: 'A: / B: 3–6 lines. Make it feel like real people. Keep it tight.',
    anglePrompts: ['friend asks “why Base?”', 'builder chat', 'team shipping banter'],
    maxTokens: 170,
    maxChars: 420,
  },
  {
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // Recent history (best-effort) lets us avoid repeating the same topic/style.
    ...
    // If it forgot to mention Base, rewrite once to add it subtly.
    if (!/\bbase\b/i.test(text)) {
      text = await openaiRewriteAddBase({ original: text, style: usedStyle, userId })
    }

    // Similarity guard: if it's too close to a recent output, retry once with a different format.
    ...
    // Store small history so the next generations diversify.
