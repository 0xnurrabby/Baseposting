...
function withFetchTimeout(url: string, init: any, timeoutMs = 8000) {
...
async function generateWithFallback(prompt: string) {
  ...
  const preferred = normalizeModelName(process.env.IMAGEN_MODEL, 'gemini-2.5-flash-image')
...
async function generateImagen(prompt: string, modelOverride?: string) {
  ...
  const r = await withFetchTimeout(url, {
    method: 'POST',
    headers: {
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const imageId = saved.id
    const internalImageUrl = `/api/image?id=${encodeURIComponent(saved.id)}`
    const publicBlobUrl = await maybeUploadToBlob(img.mimeType, img.bytesBase64Encoded).catch(() => null)
    const imageUrl = publicBlobUrl || internalImageUrl
    const imageDataUrl = `data:${img.mimeType || 'image/png'};base64,${img.bytesBase64Encoded}`
    ...
    const after = await adjustCredits(userId, -5)
    charged = true
    ...
    if (charged) {
      try { await adjustCredits(userId, +5) } catch { /* ignore */ }
    }

    const rawMsg = String(e?.message || '')
    const rawData = String((e as any)?.data?.error || '')

    // Common signals for quota/rate exhaustion from Google Generative Language API.
    // Example: "Imagen error: 429 { ... \"status\":\"RESOURCE_EXHAUSTED\" ... }"
    const looksLikeQuota429 =
      rawMsg.includes('Imagen error: 429') ||
      rawMsg.includes('Nano Banana error: 429') ||
      rawMsg.includes(' 429 ') ||
      rawMsg.toLowerCase().includes('resource_exhausted') ||
      rawMsg.toLowerCase().includes('quota') ||
      rawData.toLowerCase().includes('resource_exhausted')
    ...
    return json(res, 500, { error: rawMsg || 'Image generation failed' })
  }
}
