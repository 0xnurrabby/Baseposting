...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const origin = String(process.env.PUBLIC_SITE_URL || '').trim() || getOrigin(req) || ''
    const imageUrl = origin ? `${origin}/api/image?id=${encodeURIComponent(saved.id)}` : `/api/image?id=${encodeURIComponent(saved.id)}`
