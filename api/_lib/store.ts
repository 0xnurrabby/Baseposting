...
export async function getOrCreateUser(id: string): Promise<UserRecord> {
  ...
  if (!data || Object.keys(data).length === 0) {
    ...
    return { id, credits: 1, createdAt: created, updatedAt: created }
  }
...
export async function adjustCredits(id: string, delta: number): Promise<UserRecord> {
  ...
  try {
    ...
    if (delta >= 0) {
      await redis.hincrby(key, 'credits', Number(delta || 0))
    } else {
      const existing = await getOrCreateUser(id)
      const nextCredits = Math.max(0, Number(existing.credits || 0) + Number(delta || 0))
      await redis.hset(key, { credits: String(nextCredits) })
    }
    await redis.hset(key, { updatedAt: now })
    try { await rebuildAdminSummaries(redis) } catch { /* ignore */ }
    return await getOrCreateUser(id)
  } catch {
    const user = getOrCreateUserMemory(id)
    user.credits = Math.max(0, user.credits + delta)
    user.updatedAt = nowIso()
    memory.set(id, user)
    return user
