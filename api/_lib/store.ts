...
export async function getOrCreateUser(id: string): Promise<UserRecord> {
  ...
  if (!redis) {
    ...
    const rec: UserRecord = { id, credits: 20, lastGlobalGiftId: 0, createdAt: nowIso(), updatedAt: nowIso() }
    memory.set(id, rec)
    return rec
  }
  ...
  if (!data || Object.keys(data).length === 0) {
    ...
    return { id, credits: 20, lastGlobalGiftId: 0, createdAt: created, updatedAt: created }
  }

  return {
    id: data.id || id,
    credits: Number(data.credits || '0'),
    lastGlobalGiftId: Number(data.lastGlobalGiftId || '0'),
...
export async function setUser(user: UserRecord): Promise<void> {
  ...
  await redis.hset(key, {
    ...
    lastGlobalGiftId: String((user as any).lastGlobalGiftId || 0),
