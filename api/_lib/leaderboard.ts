...
// ISO week (Mon..Sun) helpers in UTC.
// We use this for "Previous week" so it always means the last *completed* calendar week,
// not a sliding window of "8–14 days ago".
function utcStartOfIsoWeek(dUtcMidnight: Date): Date {
  // JS: getUTCDay() => Sun=0, Mon=1, ... Sat=6
  // ISO: Mon=1, ... Sun=7
  const js = dUtcMidnight.getUTCDay()
  const iso = js === 0 ? 7 : js
  // Move back to Monday
  return datePlusDays(dUtcMidnight, -(iso - 1))
}
...
export async function recomputeLeaderboards(): Promise<{ ok: boolean; updatedAt?: string; error?: string }> {
  ...
  // Previous week (Mon..Sun, UTC): last completed ISO week.
  // Example (today = Tue): previous week is the Monday..Sunday that ended two days ago.
  const startOfThisIsoWeek = utcStartOfIsoWeek(today)
  const startPrevIsoWeek = datePlusDays(startOfThisIsoWeek, -7)
  // Fallback (if the calendar week is empty, e.g. very early after launch): use the
  // previous 7-day window right before the "last 7 days" window.
  const startPrevSliding = datePlusDays(today, -13)
  ...
  let prevStart = startPrevIsoWeek
  let topPrev = await build(prevStart, 7)
  if (topPrev.length === 0) {
    prevStart = startPrevSliding
    topPrev = await build(prevStart, 7)
  }
  ...
  await redis.hset(LB_META_KEY, {
    ...
    rangePrev: `${utcDateKey(prevStart)}..${utcDateKey(datePlusDays(prevStart, 6))}`,
  })
