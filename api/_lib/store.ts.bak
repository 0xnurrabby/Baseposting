...
function recentListKey(userId: string, kind: RecentKey) {
  return `recent:${kind}:${userId}`
}

export async function getRecent(userId: string, kind: RecentKey, limit = 12): Promise<any[]> {
  const redis = getRedis()
  if (!redis) {
    const k = recentListKey(userId, kind)
    const arr = memoryRecent.get(k) || []
    return arr.slice(0, Math.max(0, Math.min(50, limit)))
  }

  const k = recentListKey(userId, kind)
  try {
    const out = await redis.lrange(k, 0, Math.max(0, Math.min(49, limit - 1)))
...
export async function pushRecent(userId: string, kind: RecentKey, value: any, max = 20): Promise<void> {
  const redis = getRedis()
  const safeMax = Math.max(5, Math.min(60, Number(max) || 20))
  const k = recentListKey(userId, kind)

  if (!redis) {
    const cur = memoryRecent.get(k) || []
    cur.unshift(value)
    memoryRecent.set(k, cur.slice(0, safeMax))
    return
  }

  try {
    await redis.lpush(k, JSON.stringify(value))
    await redis.ltrim(k, 0, safeMax - 1)
  } catch {
    // ignore
  }
}
