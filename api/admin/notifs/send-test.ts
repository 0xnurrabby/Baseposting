...
function appUrl() {
  const raw = process.env.NOTIF_APP_URL || process.env.APP_URL || process.env.NEXT_PUBLIC_APP_URL || 'https://basepostingbro.vercel.app'
...
export default async function handler(req: any, res: any) {
  ...
  const payload = {
    ...
    targetUrl: appUrl(),
    tokens: [rec.token],
    data: { fid: rec.fid, appFid: rec.appFid, test: true },
  }
