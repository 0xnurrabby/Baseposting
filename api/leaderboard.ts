...
function hubBaseUrls(): string[] {
  ...
  const raw =
    ...
    // NOTE: Many hubs expose the HTTP API on port 2281.
    // If a hub hostname is provided without a port, we auto-append :2281 for known hubs.
    // This fixes "username/pfpUrl = null" issues when the app accidentally calls 443.
    'https://nemes.farcaster.xyz:2281,https://hub.farcaster.xyz:2281,https://hub.pinata.cloud'
  ...
  // Auto-fix common misconfiguration: missing :2281 on Farcaster hubs.
  return out.map((s) => {
    try {
      const u = new URL(s)
      const host = u.hostname.toLowerCase()
      const hasPort = Boolean(u.port)
      if (!hasPort && (host === 'nemes.farcaster.xyz' || host === 'hub.farcaster.xyz')) {
        u.port = '2281'
...
async function getFarcasterProfile(fid: number, opts?: { refresh?: boolean }) {
  ...
  if (redis) {
    try {
      if (refresh) {
        // Ensure refresh actually refreshes.
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    await Promise.all(
      uniqFids.map((fid) =>
        limit(async () => {
          const p = await getFarcasterProfile(fid, { refresh }).catch(() => null)
